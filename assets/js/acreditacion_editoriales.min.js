...
$(document).on("click",".btn_eliminar_editorial",function(){let a=$(this).attr("id_editorial");$.ajax({url:api_eliminar_editorial,type:"GET",data:"idEditorial="+a,dataType:"json",success:function(){swal("Editorial Eliminada!","La editorial se elimin\u00f3 con exito!","success");$("#tbl_editoriales").DataTable().ajax.reload(null,!1)},error:function(a){swal("Error!","La editorial no se pudo eliminar con exito!","error");console.log(a)}})});
function habilitarAgregarSello(){$(".btn_agregar_sello").off("click");$(".btn_agregar_sello").click(function(){let a=$(this).parent().parent();quitarBtnAgregarSello(this);crearInputGroup(a);$(this).remove();habilitarAgregarSello();habilitarEliminarSello()})}
...
$("#frmAcreditarEditorial").on("submit",function(a){a.preventDefault();a=extraerDatos();$.ajax({async:!0,type:"POST",dataType:"json",contentType:"application/x-www-form-urlencoded; charset=UTF-8",url:api_acreditar_editorial,data:a,success:function(a){swal("Registro Exitoso!","La editorial se acredito con exito","success");$("#frmAcreditarEditorial").trigger("reset");limpiarSellos();$("#tbl_editoriales").DataTable().ajax.reload(null,!1)},error:function(a){swal("Error en el registro!","La editorial no se pudo acreditar con exito",
"error")}})});
function extraerDatos(){return{razonSocial:$("#txt_razon_social").val(),grupoEditorial:$("#txt_grupo_editorial").val(),sellos:extraerSellos(),nombreDirectorGeneral:$("#txt_nombre_director_general").val(),emailDirectorGeneral:$("#txt_email_director_general").val(),celularDirectorGeneral:$("#txt_celular_director_general").val(),nombreRepresentanteEditorial:$("#txt_nombre_representante_editorial").val(),cargoRepresentanteEditorial:$("#txt_cargo_representante_editorial").val(),emailRepresentanteEditorial:$("#txt_email_representante_editorial").val(),observaciones:$("#txt_observaciones").val()}}
