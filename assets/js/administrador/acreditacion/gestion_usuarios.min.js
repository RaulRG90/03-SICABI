...
btn_agregar_usuario=a});$.getScript(componente_tabs_usuarios,function(){let a=$("#section_tabs");$(a).append(tabs.render(["Acreditador","Editorial"]));tabs_usuarios=a});$.getScript(componente_tablas_usuarios,function(){$("#sand_box");$("#pane_acreditador").append(tablas.render("acreditador"));tabla_acreditador=c($("#tbl_acreditador"),api_leer_acreditadores);$("#pane_editorial").addClass("active show");$("#pane_editorial").append(tablas.render("editorial"));tabla_editorial=c($("#tbl_editorial"),api_leer_editoriales);
...
if(!1===this.checkValidity())this.classList.add("was-validated");else{let a={},b=$("#txt_nombre").val(),d=$("#txt_usuario").val(),e=$("#txt_contrasenia").val();a.usu_nombre=b;a.usu_login=d;a.id_perfil=2;a.usu_pass=e;$.ajax({url:api_crear_usuario,type:"POST",dataType:"JSON",data:a,contentType:"application/x-www-form-urlencoded; charset=UTF-8",success:function(a){console.log(a);a.error?swal("Error!",a.error.message,"error"):(swal("Exito!",a.message,"success"),tabla_editorial.ajax.reload(null,!1),tabla_acreditador.ajax.reload(null,
...
$(document).on("click","#btn_editar_usuario",function(){$("#frm_editar_usuario").trigger("reset");let a=$(this).closest("tr");a.find("td:eq(0)").html();let b=$(a.find("td:eq(3)").html());$("#btn_actualizar_usuario").attr("id_usuario",$(this).attr("id_usuario"));$("#frm_editar_usuario").trigger("reset");$("#edit_txt_nombre").val(a.find("td:eq(0)").text());$("#edit_txt_usuario").val(a.find("td:eq(1)").text());$("#edit_txt_contrasenia").val($(b).val());$("#edit_txt_confirmar").val($(b).val());$("#mdl_editar_usuario").modal("show")});
$(document).off("submit","#frm_editar_usuario");$(document).on("submit","#frm_editar_usuario",function(){event.preventDefault();event.stopPropagation();if(!1===this.checkValidity())this.classList.add("was-validated");else{let a={},b=$("#btn_actualizar_usuario").attr("id_usuario"),d=$("#edit_select_perfil").val(),e=$("#edit_txt_nombre").val(),c=$("#edit_txt_usuario").val(),f=$("#edit_txt_contrasenia").val();a.usu_id=b;a.id_perfil=d;a.usu_nombre=e;a.usu_login=c;a.usu_pass=f;$.ajax({url:api_actualizar_usuario,
type:"POST",dataType:"JSON",data:a,contentType:"application/x-www-form-urlencoded; charset=UTF-8",success:function(a){console.log(a);a.error?swal("Error!",a.error.message,"error"):(swal("Exito!",a.message,"success"),tabla_editorial.ajax.reload(null,!1),tabla_acreditador.ajax.reload(null,!1))},error:function(a,b){swal("Error!","Error desconocido, comuniquese con el administrador","error")}})}});$(document).off("change","#txt_contrasenia");$(document).on("change","#txt_contrasenia",function(){let a=
$(this).val();$("#txt_confirmar").attr("pattern",a)});$(document).off("change","#edit_txt_contrasenia");$(document).on("change","#edit_txt_contrasenia",function(){let a=$(this).val();$("#edit_txt_confirmar").attr("pattern",a)})});